...
trainedNet = train(net,trainSet,targets);



testingCurves = NNUtils.getTestingSet(Zcluster,3,total,trainAmount);
testing = trainedNet(testingCurves);
classes = vec2ind(testing);

NNUtils.verifyResults(classes,total,trainAmount,3,1);
NNUtils.verifyResults(classes,total,trainAmount,3,2);
NNUtils.verifyResults(classes,total,trainAmount,3,3);
%%
...
Z = normalize(realPart+1i*imagPart);
...
minR = min(Zr);
%     minI = min(abs(Zi);
impedance = ((Zr-minR)/maxR)+1i*(Zi/maxI);

end


function Z = NewModel(params, freq)
Z = FouquetModel(params, freq);
end
