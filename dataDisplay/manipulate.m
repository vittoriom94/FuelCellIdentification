function manipulate(mathfunc_full, XYLim, refX,refY,varargin)
% ho aggiunto refX e refY, se non voglio usarli metto [],[]
...
errorObjective = objectiveF([real(data);imag(data)],[refX;refY]);
error_text = uicontrol('style','text','Units','pixels',...
    'Position',[0 axes_pos_p(2) + axes_pos_p(4) + ...
        (n_params2man ).*(slider_spacing + slider_thickness) 200 50],...
    'String',num2str(errorObjective));
for i = 1:n_params2man    
...
%         x = get(plot_h,'XData');
        ...
        set(plot_h, 'YData', -imag(y));
        set(plot_h,'XData',real(y));
        set(plot_h, 'UserData', params);
        set(paramval_h(iParam), 'String', num2str(params(iParam)));
        errorObjective = objectiveF([real(y);imag(y)],[refX;refY]);
        set(error_text,'String',num2str(errorObjective));
        drawnow;
