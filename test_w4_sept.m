...
% TODO: invece di splittare sulla stessa colonna, splittare su due colonne
...
%%
result = struct('resid',[],'conf',[],'params',[]','minparams',[],'index',[],'outReason',[],'time',[]);
[result.resid,result.conf,result.params,result.minparams,result.index,result.outReason,result.time] = fitMultiple(-real(Znoise),-imag(Znoise),fullData{2,1}.FrequencyHz,lowerBound,upperBound,data);
smallestResid = result.resid(result.index)
dataSize = length(fullData{2,1}.FrequencyHz);
rmse = sqrt(smallestResid/dataSize)   
